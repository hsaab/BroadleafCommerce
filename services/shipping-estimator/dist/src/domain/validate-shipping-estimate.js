...
export function parseShippingEstimateRequest(input) {
    const fields = [];
    if (!isRecord(input)) {
        return {
            fields: [
    ...
    if (Array.isArray(items) && Array.isArray(input.options)) {
        validateOptionItemRequirements(items, input.options, fields);
    }
    return {
        request: fields.length === 0 ? input : undefined,
        fields,
    };
}
function validateCurrency(value, fields) {
...
function validateItems(value, fields) {
...
function validateOptions(value, fields) {
...
function validateBands(value, optionType, optionPath, fields) {
    if (!Array.isArray(value) || value.length === 0) {
        fields.push({
...
function validateOptionItemRequirements(items, options, fields) {
...
function validateConfiguredBands(option, path, fields) {
...
function validateWeight(value, path, fields) {
    if (!isRecord(value)) {
        fields.push({
            path,
            code: "INVALID_TYPE",
...
function validateFlatRates(value, path, fields) {
...
function validateOptionalDecimal(value, path, fields) {
    if (value !== undefined) {
        validateRequiredDecimal(value, path, fields);
    }
}
function validateRequiredDecimal(value, path, fields) {
...
function hasFlatRateForOption(item, optionId) {
