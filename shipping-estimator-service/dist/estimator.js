...
exports.validateEstimateRequest = validateEstimateRequest;
...
function validateFulfillmentBand(band, index, errors) {
...
function validateFulfillmentOption(option, index, errors) {
    if (typeof option !== 'object' || option === null) {
        errors.push(`options[${index}] must be an object`);
        return;
    }
    const maybeOption = option;
...
function validateEstimateRequest(body) {
    ...
    return { valid: errors.length === 0, errors, request: errors.length === 0 ? request : undefined };
}
function computeFulfillmentOptionPrice(option, request) {
    ...
    let flatTotal = new big_js_1.default(0);
    let retailTotal = new big_js_1.default(0);
    let weightTotal = new big_js_1.default(0);
    ...
    for (const item of items) {
        ...
        if (option.type === 'bandedPrice') {
            const itemPrice = item.price ? parseDecimal(item.price) : new big_js_1.default(0);
            retailTotal = retailTotal.plus(itemPrice.times(item.quantity));
        }
        if (option.type === 'bandedWeight') {
            const itemWeight = item.weight ? parseDecimal(item.weight) : new big_js_1.default(0);
            const weightUnit = item.weightUnit ?? 'POUNDS';
            const convertedWeight = convertWeight(itemWeight, weightUnit).times(item.quantity);
            weightTotal = weightTotal.plus(convertedWeight);
        }
    }
    let lowestBandPrice = null;
    let lowestBandMinimum = new big_js_1.default(0);
    if (foundCandidateForBand) {
        for (const band of bands) {
            const bandMinimum = parseDecimal(band.minimum);
            const matched = option.type === 'bandedPrice'
    ...
    if (lowestBandPrice === null) {
        lowestBandPrice = new big_js_1.default(0);
    }
    return formatMoney(lowestBandPrice.plus(flatTotal));
}
function estimateShipping(request) {
    return {
